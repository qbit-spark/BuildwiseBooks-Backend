...
public interface ProjectRepo extends JpaRepository<ProjectEntity, UUID> {
    ...
    @Query("SELECT p FROM ProjectEntity p LEFT JOIN FETCH p.projectSubcontractors ps LEFT JOIN FETCH ps.subcontractor WHERE p.projectId = :projectId")
    Optional<ProjectEntity> findByIdWithSubcontractors(@Param("projectId") UUID projectId);
}